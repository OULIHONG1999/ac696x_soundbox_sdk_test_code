...
const uint8_t tuya_mac_addr[6] = {0xDC, 0x23, 0x4D, 0x65, 0xB0, 0x66}; //The actual MAC address is : 66:55:44:33:22:11
...
void tuya_data_init()
{
    ...
    int ret = tuya_ble_dp_data_send(sn, DP_SEND_TYPE_ACTIVE, DP_SEND_FOR_CLOUD_PANEL, DP_SEND_WITH_RESPONSE, &p_dp_data, 5);
    if (ret != 0) {
        printf("tuya_ble_dp_data_send ret = %d", ret);
    }
#endif
}
...
static void tuya_device_reset(void)
{
    printf("tuya unbound and clean data");
    tuya_led_state = 0;
    gpio_direction_output(LED_PIN, tuya_led_state);
}
...
static void tuya_cb_handler(tuya_ble_cb_evt_param_t *event)
{
    ...
    case TUYA_BLE_CB_EVT_DATA_PASSTHROUGH:
        ...
        break;
    default:
        printf("app_tuya_cb_queue msg: unknown event type 0x%04x", event->evt);
        break;
    }
...
void tuya_ble_app_init(void)
{
    ...
    if (device_param.device_id_len == 16) {
        ...
        memcpy(device_param.mac_addr.addr, tuya_mac_addr, 6);
