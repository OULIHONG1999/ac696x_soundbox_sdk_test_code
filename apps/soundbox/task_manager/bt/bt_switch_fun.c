...
int bt_must_work(void)
{
    ...
    if ((get_call_status() == BT_CALL_OUTGOING) || (get_call_status() == BT_CALL_ALERT) || (get_call_status() == BT_CALL_INCOMING) || (get_call_status() == BT_CALL_ACTIVE))
    {
        // 通话不退出
        return true;
    }
...
int a2dp_media_packet_user_handler(u8 *data, u16 size)
{
    ...
    int energy = 0;
    if (media_type == 0 /* A2DP_CODEC_SBC */)
    { // 后台返回蓝牙
